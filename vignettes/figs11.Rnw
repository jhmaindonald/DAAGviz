...
%\VignetteIndexEntry{Brief Notes on Text Mining (Set 11)}
...
\title{11: Brief Notes on Text Mining}
...
fig11.1 <- function(){
    cat("\n",
        "Run the separate functions fig11.1A(), fig11.1B() & fig11.1C()",
        "\n")
}
@ %

\begin{figure}
<<wc-14_1x, fig.width=5.75, fig.height=5.75, cache=FALSE, out.width="0.32\\textwidth", top=5, eval=doFigs>>=
fig11.1A()
fig11.1B()
fig11.1C()
@ %
