...
quickplot(Year, mdbRain, data=bomregions2011,
          geom=c("point","smooth"), se=TRUE,
          method="loess", span=0.5, xlab="",
          ylab="Av. rainfall, M-D basin")

## ---- bom2012 ----
ggplot(bomregions2012, aes(x=Year, y=mdbRain)) +
...
## ---- qplotADDgg ----
qplot(Year, mdbRain, data=bomregions2012,
      geom="point",
...
quickplot(Year, mdbRain, data=bomregions2012,
...
quickplot(wt, ht, xlab="Weight (kg)",
          ...
          facets = . ~ sex) +
  geom_boxplot(outlier.size=1.75,
               outlier.colour="gray",
               color="gray") +
