...
## ---- download-bitr ----
url <- paste0("http://www.bitre.gov.au/publications/ongoing/files/",
              "WebAirport_CY_1985-2013.xls")
download.file(url, destfile="auAirTraffic.xls", mode="wb")

## ---- read-xls ----
library(XLConnect)
wbk2 <- loadWorkbook('auAirTraffic.xls')
airTraffic <- readWorksheet(wbk2, sheet="TOP10", startRow=8, endRow=36,
                            startCol=2, endCol=9)
names(airTraffic) <- c("Year", "DomPassRev", "IntPassRev","TotPassRev", "",
                       "DomGrowth","IntGrowth","TotGrowth")
...
path2file <- memisc::UnZip(zipfile="anes/NES1948.ZIP", item="NES1948.POR",
                           package="memisc")
